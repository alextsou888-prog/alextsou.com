...
export const ui = {
  en: {
    ...
    heroRole: 'Senior Software Tool Development, System Integration & Validation Engineer',
    heroLead: 'C#/.NET and Python engineering-tool development, device/equipment control, SDK/API integration, and HW/FW/SW system integration, backed by hands-on experience in Camera SoC, Wi-Fi/5G, Automotive, AI/NPU, and system validation.',
    heroContext: 'Software Tool Development · System Integration · Device / Equipment Control · Validation',
    ...
    skillsLead: 'C#/.NET and Python software/engineering-tool development, system integration, device/equipment control, SDK/API and communication integration, with supporting System / SoC Validation, test automation, and HW/FW/SW debug experience.',
  ...
  zh: {
    ...
    eyebrow: '工程作品集',
    heroA: 'Alex Tsou', heroB: '',
    heroRole: '資深軟體工具開發、系統整合與驗證工程師',
    heroLead: '以 C#/.NET 與 Python 進行工程工具開發、設備／裝置控制、SDK/API 整合與 HW/FW/SW 系統整合，並具備 Camera SoC、Wi-Fi/5G、Automotive、AI/NPU 與系統驗證實務經驗。',
    heroContext: '軟體工具開發 · 系統整合 · 設備／裝置控制 · 驗證',
    ...
    skillsLead: '以 C#/.NET 與 Python 軟體／工程工具開發、系統整合、設備／裝置控制、SDK/API 與通訊整合為核心，並具備 System / SoC Validation、測試自動化與 HW/FW/SW Debug 實務經驗。',
...
export const capabilityOverview = {
  ...
  subtitle: c(
    'Hands-on software tool development, system integration, device/equipment control, and validation across semiconductor, embedded, wireless, camera, automotive, and AI/NPU domains.',
    '具軟體工具開發、系統整合、設備／裝置控制與驗證實務，涵蓋半導體、Embedded、Wireless、Camera、Automotive 與 AI/NPU 領域。',
  ),
  snapshot: [
    ...
    c('Software Tool Development · System Integration · Device / Equipment Control · Validation', '軟體工具開發 · 系統整合 · 設備／裝置控制 · 驗證'),
  ],
  cards: [
    {
...
const experienceCatalog: PortfolioItem[] = [
  {
    ...
    sections: [
      ...
      {
        label: c('Automotive SPICE / Polarion Validation', 'Automotive SPICE / Polarion 驗證流程'),
        bullets: {
          en: [
            'Used Siemens Polarion ALM in automotive validation activities for requirement traceability, test-case management, defect tracking, and verification evidence. Participated in Automotive SPICE-oriented validation workflows and cross-functional issue closure.',
            'Familiar with fundamental ISO 26262 functional-safety concepts including HARA, ASIL, Safety Goal, traceability, and verification evidence. My practical experience is focused on validation and evidence management rather than functional-safety architecture ownership.',
          ],
          zh: [
            '於車用驗證工作中使用 Siemens Polarion ALM，進行 Requirement Traceability、Test Case 管理、Defect Tracking 與 Verification Evidence 整理，並參與 Automotive SPICE 導向的驗證流程與跨部門問題結案。',
            '具 ISO 26262 Functional Safety 基本概念，了解 HARA、ASIL、Safety Goal、Traceability 與 Verification Evidence；實務重點在 Validation 與 Evidence 管理，不宣稱 Functional Safety Architecture ownership。',
          ],
        },
      },
  ...
  },
  {
    ...
    tags: ['Python Engineering Tool', 'SDK Integration', 'Camera SoC', 'AI/NPU', 'Wi-Fi FPGA', 'IC / SoC Validation', 'Linux', 'Shell Script', 'Kubernetes (K8s)'],
    career: {
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: {
          en: ['Developed a Python Camera / IPCam SDK engineering tool with pytest automation for IP Camera and Video SoC SDK validation', 'Integrated PySerial, SDK / CGI, UART, Linux shell, relay, and power-control interfaces for camera parameter control', 'Controlled H.264 / H.265 modes, resolution, FPS, GOP, bitrate, and CBR / VBR behavior', 'Used Python + OpenCV for image capture / recording, frame extraction, data processing, image comparison, rotation handling, ROI processing, and SSIM-based image-quality analysis.', 'Automated cold boot, warm boot, stress, stability, memory-leak monitoring, and firmware programming', 'Used an existing Python AI/NPU binary pattern generation flow and performed validation across Simulator / FPGA / Target environments.', 'Compared output.bin with golden.bin and automated numerical PASS / FAIL plus regression evidence', 'Supported repeatable Audio AI validation with Python, Gradio, Docker, datasets, and scripts', 'Used Linux shell commands and shell scripts for DUT control and validation automation.', 'Used Kubernetes-based test environments to execute test jobs, inspect execution logs, track test results, and support issue analysis for IP Camera validation.'],
          zh: ['開發 Python Camera / IPCam SDK 工程工具，並以 pytest 建立 IP Camera 與 Video SoC SDK 驗證自動化', '整合 PySerial、SDK / CGI、UART、Linux Shell、Relay 與電源控制介面，進行 Camera 參數控制', '控制 H.264 / H.265 模式、解析度、FPS、GOP、Bitrate 與 CBR / VBR 行為', '於 Novatek 使用 Python + OpenCV 進行影像擷取／錄影、Frame Extraction、資料處理、影像比對、Rotation 處理、ROI Processing 與以 SSIM 為基礎的影像品質分析。', '自動化 Cold Boot、Warm Boot、Stress、穩定度、Memory Leak 監控與 Firmware Programming', '使用既有 Python AI/NPU Binary Pattern Generator／Pattern Generation Flow，並於 Simulator / FPGA / Target 環境執行驗證。', '比對 output.bin 與 golden.bin，並自動產生數值 PASS / FAIL 與回歸證據', '以 Python、Gradio、Docker、資料集與腳本支援可重複的 Audio AI 驗證', '使用 Linux Shell 指令與 Shell Script 進行 DUT 控制與驗證自動化。', '使用 Kubernetes（K8s）測試環境執行 Test Job、查看執行 Log，追蹤測試結果並協助 IP Camera 驗證異常分析。'],
        },
      },
      {
      ...
      {
        label: c('AI/NPU Validation & Golden Result Comparison', 'AI/NPU 驗證與 Golden Result 比對'),
        body: c('Performed AI/NPU validation using an existing binary pattern generation flow and Golden Result Comparison across Simulator / FPGA / Target environments. Verified output consistency using bit-accurate comparison where applicable, and performed log analysis and failure investigation for mismatched results.', '執行 AI/NPU 驗證，使用既有 Binary Pattern Generator／Pattern Generation Flow 產生測試資料，針對 Simulator / FPGA / Target 與 Golden Result 進行結果比對；依驗證階段進行 bit-accurate comparison，並針對 mismatch 執行 Log Analysis 與 Failure Analysis。'),
      ...
      section('Tech Stack', '技術工具', 'Python · SDK Integration · Linux · Shell Script · pytest · PySerial · SCPI · VeriWave · OpenCV · FFmpeg · Gradio · Docker · Kubernetes (K8s) · Jenkins · GitLab CI', 'Python · SDK Integration · Linux · Shell Script · pytest · PySerial · SCPI · VeriWave · OpenCV · FFmpeg · Gradio · Docker · Kubernetes (K8s) · Jenkins · GitLab CI'),
    ],
  },
  {
...
export const certificationItem: PortfolioItem = {
  ...
  summary: c(
    'Wi-Fi certification testing and SGS evidence preparation · WHQL execution · Android CTS / GTVS testing, issue analysis, fix verification, and regression support.',
    'Wi-Fi 認證測試與 SGS 證據整理 · WHQL 測試執行 · Android CTS / GTVS 測試、問題分析、修正驗證與回歸支援。',
  ),
  tags: ['Wi-Fi', 'WHQL', 'Android CTS / GTVS'],
  sections: [
    {
...
export const careerSnapshot = {
  ...
  summary: c(
    'Software tool development engineer (C#/.NET, Python) with hands-on experience in device/equipment control, SDK/API and communication integration, and HW/FW/SW system integration, plus debug and root-cause analysis. Background spans Camera SoC, AI/NPU, Wi-Fi FPGA / Chip engineering, and embedded product integration, with supporting system validation and test automation experience. Interested in Software Tool Development, System Integration, and System Validation roles within semiconductor and embedded product development.',
    '軟體工具開發工程師（C#/.NET、Python），具備設備／裝置控制、SDK/API 與通訊整合、HW/FW/SW 系統整合，以及 Debug 與根因分析實務經驗。技術背景涵蓋 Camera SoC、AI/NPU、Wi-Fi FPGA / Chip Engineering 與 Embedded Product Integration，並具備系統驗證與測試自動化支援經驗，下一階段聚焦軟體工具開發、系統整合與系統驗證相關職務。',
  ),
  facts: [
    {
      ...
      values: [
        ...
        c('Software Tool / Engineering Tool Development Engineer', '軟體工具 / 工程工具開發工程師'),
        c('System Integration Engineer', '系統整合工程師'),
        c('System Validation Engineer', '系統驗證工程師'),
        c('Technical Lead', '技術主管'),
      ],
    },
...
export const resumeCoreGroups = [
  ...
  { title: c('Experience Overview', '經歷總覽'), items: [c('FIH engineering experience, Novatek validation work, TPV FAE support, and earlier system-validation management experience are represented in the interactive timeline.', 'FIH 工程經歷、聯詠驗證工作、冠捷 FAE 支援，以及早期系統驗證管理經驗已整理於互動時間軸。')] },
  {
    ...
    items: [
      c(
        'For my next role, I am focusing on Senior Software Engineer, Software / Engineering Tool Development, System Integration, System Validation, and Technical Lead opportunities where I can apply C#/.NET, Python, device/equipment control, SDK/API integration, HW/FW/SW integration, automation, and Debug/RCA experience.',
        '下一階段希望投入資深軟體工程師、軟體／工程工具開發、系統整合、系統驗證或 Technical Lead 相關職務，持續運用 C#/.NET、Python、設備／裝置控制、SDK/API 整合、HW/FW/SW Integration、自動化與 Debug/RCA 經驗。',
      ),
    ],
  },
];
...
export const flagshipCaseStudies: PortfolioItem[] = [
  {
    ...
    summary: c('AI/NPU validation using an existing pattern-generation flow: controlled target execution, golden comparison, PASS / FAIL review, and regression evidence.', '使用既有 Pattern Generation Flow 執行 AI/NPU 驗證：受控目標執行、Golden 比對、PASS / FAIL 確認與回歸證據。'),
    tags: ['Python', 'Pattern.bin', 'FPGA / NPU', 'Golden Output', 'Mismatch Analysis', 'Regression'],
    sections: [
      section('Overview', '案例概述', 'Within one continuous Novatek employment period, this work focused on hands-on AI / NPU validation using existing test patterns / pattern-generation flow, controlled execution, trusted-output comparison, automated PASS / FAIL, and regression.', '此工作位於聯詠同一段連續任職期間內，聚焦 AI / NPU 實作驗證：使用既有測試 Pattern／Pattern Generation Flow、受控執行、可信結果比對、自動 PASS / FAIL 與回歸測試。'),
      ...
      section('My Role', '我的角色', 'Performed AI/NPU validation using an existing pattern-generation flow, executed test inputs across Simulator / FPGA / Target environments, compared outputs against Golden results, reviewed PASS / FAIL results, and analyzed mismatches and logs.', '使用既有 Pattern Generation Flow 執行 AI/NPU 驗證，於 Simulator / FPGA / Target 環境執行測試資料，將輸出結果與 Golden Result 比對，確認 PASS / FAIL，並分析 mismatch 與 Log。'),
      { label: c('Validation Activities', '驗證工作'), bullets: { en: ['Existing binary test-pattern flow execution', 'Simulator / FPGA / Target validation', 'output.bin vs golden.bin comparison', 'Bit-accurate / numerical result verification where applicable', 'Mismatch log and failure analysis', 'PASS / FAIL result review', 'Regression validation'], zh: ['既有 Binary Test Pattern Flow 執行', 'Simulator / FPGA / Target 驗證', 'output.bin 與 golden.bin 比對', '依驗證階段執行 bit-accurate / numerical result verification', 'Mismatch Log 與 Failure Analysis', 'PASS / FAIL 結果確認', 'Regression Validation'] } },
      ...
      { label: c('Engineering Outputs', '工程輸出'), bullets: { en: ['Execution and comparison records', 'Mismatch evidence', 'PASS / FAIL result records', 'Regression reports'], zh: ['執行與比對紀錄', 'Mismatch 證據', 'PASS / FAIL 結果紀錄', '回歸報告'] } },
...
export const visualItems: PortfolioItem[] = [
  ...
  },
  {
    ...
    summary: c('Visual brief for AI/NPU model validation using an existing pattern-generation flow, simulator/FPGA execution, golden comparison, and regression testing.', 'AI/NPU 驗證圖解涵蓋使用既有 Pattern Generation Flow、Simulator / FPGA 執行、Golden 比對與回歸測試。'),
    ...
    sections: [
      ...
      section('My Role', '我的角色', 'Performed AI/NPU validation using an existing test-pattern generation flow, controlled execution, numerical comparison, regression testing, and validation reporting.', '使用既有測試 Pattern Generation Flow 執行 AI / NPU 驗證工作，涵蓋受控執行、數值比對、回歸測試與驗證報告。'),
      ...
      section('Validation Activities', '驗證工作', 'Existing binary test-pattern flow execution, output.bin-to-golden.bin comparison, mismatch review support, PASS / FAIL handling, and regression reporting.', '既有 Binary Test Pattern Flow 執行、output.bin 與 golden.bin 比對、Mismatch Review 支援、PASS / FAIL 處理及回歸報告。'),
